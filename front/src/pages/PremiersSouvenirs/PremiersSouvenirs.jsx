...
import styles from './premiersSouvenirs.module.css' // Assurez-vous de créer ce fichier CSS module

const PremiersSouvenirs = () => {
  // Récupérer les articles du store Redux
  const store = useSelector((state) => state.article.data)
  const loading = useSelector((state) => state.article.loading)

  // je filtre uniquement les articles de catégorie premiers souvenirs
  const premiersSouvenirsArticles = Array.isArray(store)
    ? store.filter(article => article.categorie === 'Premiers Souvenirs')
    : []
  ...
  return (
    <>
      ...
      <div className={styles.headerContainer}>
        <h1>Premiers Souvenirs</h1>
      </div>
      <div className={styles.infoContainer}>
        <div className={styles.blockTitre}>
          <h3 className={styles.h3}>Premiers Souvenirs...</h3>
      ...
      {/* Affichage des articles premiers souvenirs */}
      <div className="container py-4">
        {premiersSouvenirsArticles.length > 0 ? premiersSouvenirsArticles.map((article) => (
        ...
        )) : <p>Aucun article premiers souvenirs trouvé</p>}
      </div>
    </>
  )
}

export default PremiersSouvenirs