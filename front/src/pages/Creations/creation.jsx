...
import styles from './Creations.module.css' // Assurez-vous de créer ce fichier CSS module
...
const Creation = () => {
  ...
  // je filtre uniquement les articles de catégorie creations
  const creationsArticles = Array.isArray(store)
    ? store.filter(article => article.categorie === 'Création')
    : []
    ...
    <>
      ...
      <div className={styles.infoContainer}>
        <div className={styles.blockTitre}>
          <h3 className={styles.h3}>Créations...</h3>
      ...
      {/* Affichage des articles creations */}
      <div className="container py-4">
        {creationsArticles.length > 0 ? creationsArticles.map((article) => (
            ...
            <div className="col-md-5">
              <div className={styles.productImgContainer}>
                {article.photo && article.photo.length > 0 ? (
                  <img src={article.photo[0]} className={styles.productImg} alt={article.nom || 'Image Création'} />
                ) : (
        ...
        )) : <p>Aucun article Création trouvé</p>}
      </div>
    </>
  )
}
