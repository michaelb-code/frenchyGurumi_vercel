...
export const createAvis = async (req, res) => {
    try {
        ...
        //user fourni un nom au lieu dun id:
        if (mongoose.Types.ObjectId.isValid(userId)) {
            ...
            if (mongoose.Types.ObjectId.isValid(articleId)) {
                ...
                article = await Article.findOne({ name: articleId });
            }
            if (!article) {
...
export const getAvisByArticleName = async (req, res) => {
    try {
        ...
        res.status(200).json({ avis, message: `${avis.length} avis trouvés pour l'article ${article.name}` });
    } catch (error) {
        console.log(error, "Erreur lors de la recuperation de l'avis");
        return res.status(500).json({ error: error.message });
    }
...
export const deleteAvis = async (req, res) => {
    try {
        ...
        res.status(200).json({
            ...
            name: avis.article.name, //nom de l'article qui a recu l'avis modifié
            message: "Votre avis a étésupprimé avec succés"
        });
    } catch (error) {
        console.log(error, "Erreur lors de la suppression de l'avis");
        return res.status(500).json({ error: error.message, message: "Erreur lors de la suppression de l'avis" });
    }
};