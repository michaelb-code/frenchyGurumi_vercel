    ...
    try {
        ...
        if (fieldsMissing) {
            return res.status(400).json({
                message: `Le champ ${fieldsMissing} est obligatoire pour la création d'un article`,
            });
        }
        if (!req.files || req.files.length === 0) {
            return res.status(400).json({
                message: "Au moins une photo est requise pour créer un article",
            });
        }
        ...
        const photoUrls = req.files.map(
            (file) => file.secure_url || baseUrl + file.path.replace(/\\/g, "/")
        );
        ...
        res
            .status(201)
            .json({ message: "Article créé avec succés", data: newArticle });
    } catch (error) {
...
export const getArticleById = async (req, res) => {
    try {
        ...
        res
            .status(200)
            .json({ article, message: `Article  ${article.nom}  trouvé` });
    } catch (error) {
...
export const updateArticle = async (req, res) => {
    try {
        ...
        // Garder l'ancienne photo si aucune nouvelle image n'est envoyée
        let photoUrls = article.photo;

        if (req.files && req.files.length > 0) {
            const baseUrl = `${req.protocol}://${req.get("host")}/`; // Génère http://localhost:8000/
            photoUrls = req.files.map((file) => baseUrl + file.path.replace(/\\/g, "/")); // Remplace "\" par "/"
        }
        ...
        res.status(200).json({
            ...
            message: `Article ${updatedArticle.nom} modifié avec succès`,
        });
    } catch (error) {
...
export const deleteArticle = async (req, res) => {
    try {
        ...
        res.status(200).json({
            article,
            message: `Article  ${article.nom}  supprimé avec succés`,
        });
    } catch (error) {
